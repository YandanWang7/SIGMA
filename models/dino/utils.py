...
def load_class_freq(
        path='data/annotations/lvis_v1_train_cat_info.json',
        freq_weight=1.0):
    candidates = []
    if path:
        candidates.append(Path(path))

    for env_name in ("LVIS_PATH", "LVIS_ROOT", "COCO_PATH", "DATASET"):
        env_value = os.environ.get(env_name)
        if env_value:
            candidates.append(Path(env_value) / "annotations" / "lvis_v1_train_cat_info.json")

    candidates.append(Path("data/annotations/lvis_v1_train_cat_info.json"))
    ...
    if resolved_path is None:
        raise FileNotFoundError(
            "Could not locate lvis_v1_train_cat_info.json. Checked: {}".format(checked)
        )

    cat_info = json.load(open(resolved_path, 'r'))
